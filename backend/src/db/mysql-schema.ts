...
export const mysqlSchemaStatements = [
  ...
  `CREATE TABLE IF NOT EXISTS sys_task (
    ...
    credit_status VARCHAR(32) DEFAULT 'none',
    credit_workspace_id INT,
    credit_user_id INT,
  ...
  `CREATE TABLE IF NOT EXISTS workspaces (
    ...
    created_at VARCHAR(64) NOT NULL,
    updated_at VARCHAR(64) NOT NULL,
    INDEX idx_workspaces_owner_user_id (owner_user_id)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
  ...
  `CREATE TABLE IF NOT EXISTS drama_members (
    ...
    role VARCHAR(32) NOT NULL DEFAULT 'creator',
    created_at VARCHAR(64) NOT NULL,
    UNIQUE KEY uk_drama_members_user (drama_id, user_id),
    INDEX idx_drama_members_user_id (user_id)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
  ...
  `CREATE TABLE IF NOT EXISTS credit_accounts (
    ...
    created_at VARCHAR(64) NOT NULL,
    updated_at VARCHAR(64) NOT NULL,
    UNIQUE KEY uk_credit_accounts_workspace_id (workspace_id)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
  ...
  `CREATE TABLE IF NOT EXISTS credit_ledger (
    ...
    INDEX idx_credit_ledger_workspace_id (workspace_id)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
  ...
  `CREATE TABLE IF NOT EXISTS pricing_rules (
    ...
    params TEXT,
    price INT NOT NULL,
    unit VARCHAR(32) NOT NULL DEFAULT 'task',
    is_active TINYINT(1) DEFAULT 1,
    created_at VARCHAR(64) NOT NULL,
    updated_at VARCHAR(64) NOT NULL,
    INDEX idx_pricing_rules_action (action),
  ...
  `CREATE TABLE IF NOT EXISTS recharge_orders (
    ...
    INDEX idx_recharge_orders_workspace_id (workspace_id)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
...
export const stylePresetSeeds = [
  { name: '3D 漫剧', value: '3d', sortOrder: 1, prompt: 'stylized 3D animated drama style, high-quality game-engine render, clearly fictional non-photorealistic characters, animated film character design, soft sculpted facial features, expressive but simplified eyes, clean skin without photo-real pores, painterly PBR materials, cinematic animation lighting, no real person, no photorealistic portrait, no live-action photo look', description: '风格化 3D 动画短剧质感，弱化真人照片感，适合作为视频参考素材' },
...
export const mysqlPricingSeedStatements = pricingRuleSeeds.map((s) => ({
  sql: 'INSERT INTO `pricing_rules` (`action`, `service_type`, `price`, `unit`, `is_active`, `created_at`, `updated_at`) SELECT ?, ?, ?, ?, 1, ?, ? FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `pricing_rules` WHERE `action` = ? AND `service_type` = ?)',
  params: [s.action, s.serviceType, s.price, s.unit, new Date().toISOString(), new Date().toISOString(), s.action, s.serviceType],
}))
...
export async function initMySqlSchema(pool: Pool) {
  ...
  // 兼容已有本地数据库：新字段只补一次，保留原有项目数据。
  const [dramaColumns] = await pool.query(`SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'dramas' AND COLUMN_NAME = 'workspace_id'`) as [{ COLUMN_NAME: string }[], unknown]
  if (!dramaColumns.length) await pool.query('ALTER TABLE dramas ADD COLUMN workspace_id INT NULL AFTER id')
  const [userProfileColumns] = await pool.query(`SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'nickname'`) as [{ COLUMN_NAME: string }[], unknown]
  ...
  if (!taskWorkspaceColumns.length) await pool.query('ALTER TABLE sys_task ADD COLUMN credit_workspace_id INT NULL, ADD COLUMN credit_user_id INT NULL')
  const [ledgerIndexes] = await pool.query(`SELECT INDEX_NAME FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'credit_ledger' AND INDEX_NAME = 'uk_credit_ledger_idempotency'`) as [{ INDEX_NAME: string }[], unknown]
  if (!ledgerIndexes.length) {
    await pool.query('ALTER TABLE credit_ledger ADD UNIQUE KEY uk_credit_ledger_idempotency (idempotency_key)')
  }
  const [orderIndexes] = await pool.query(`SELECT INDEX_NAME FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'recharge_orders' AND INDEX_NAME = 'uk_recharge_orders_order_no'`) as [{ INDEX_NAME: string }[], unknown]
  if (!orderIndexes.length) {
    await pool.query('ALTER TABLE recharge_orders ADD UNIQUE KEY uk_recharge_orders_order_no (order_no)')
  }
  for (const seed of mysqlDataSeedStatements) {
    await pool.query(seed.sql, seed.params)
  }
  for (const seed of mysqlPricingSeedStatements) {
    await pool.query(seed.sql, seed.params)
  }
}
