...
import { canAccessEpisode, canWriteDrama } from '../utils/workspace-access.js'
...
app.use('/:id/*', async (c, next) => {
  ...
  if (!storyboard || !await canAccessEpisode(c, storyboard.episodeId)) return notFound(c, '镜头不存在')
  return next()
})
...
app.post('/', async (c) => {
  ...
  if (!body.episode_id || !await canAccessEpisode(c, Number(body.episode_id))) return notFound(c, '剧集不存在')
  const [episode] = await db.select({ dramaId: schema.episodes.dramaId })
    .from(schema.episodes).where(eq(schema.episodes.id, Number(body.episode_id)))
  if (!episode || !await canWriteDrama(c, episode.dramaId)) return c.json({ code: 403, message: '当前角色没有编辑镜头的权限' }, 403)
  const ts = now()
...
app.put('/:id', async (c) => {
  ...
  const [episode] = await db.select({ dramaId: schema.episodes.dramaId })
    .from(schema.episodes).where(eq(schema.episodes.id, storyboard.episodeId))
  if (!episode || !await canWriteDrama(c, episode.dramaId)) return c.json({ code: 403, message: '当前角色没有编辑镜头的权限' }, 403)
...
app.delete('/:id', async (c) => {
  ...
  const [episode] = await db.select({ dramaId: schema.episodes.dramaId })
    .from(schema.episodes).where(eq(schema.episodes.id, storyboard.episodeId))
  if (!episode || !await canWriteDrama(c, episode.dramaId)) return c.json({ code: 403, message: '当前角色没有删除镜头的权限' }, 403)
