...
import { canAccessDrama, canManageDrama, canWriteDrama } from '../utils/workspace-access.js'

const app = new Hono()

function taskEpisodeId(task: typeof schema.sysTask.$inferSelect) {
  try {
    return Number(JSON.parse(task.params || '{}')?.episode_id || 0)
  } catch {
    return 0
  }
}
...
// 所有剧集详情、编辑、生成和任务接口统一检查所属项目工作区。
app.use('/:id', async (c, next) => {
  const episodeId = Number(c.req.param('id'))
  if (!Number.isInteger(episodeId)) return next()
  const [episode] = await db.select({ episode: schema.episodes, drama: schema.dramas })
    .from(schema.episodes)
    .innerJoin(schema.dramas, eq(schema.dramas.id, schema.episodes.dramaId))
    .where(eq(schema.episodes.id, episodeId))
  if (!episode) return notFound(c, '剧集不存在')
  const workspaceId = await currentWorkspaceId(c)
  if (episode.drama.workspaceId != null && episode.drama.workspaceId !== workspaceId) return notFound(c, '剧集不存在')
  if (!await canAccessDrama(c, episode.drama.id)) return notFound(c, '剧集不存在')
  return next()
})
...
app.post('/', async (c) => {
  ...
  const [drama] = await db.select().from(schema.dramas).where(eq(schema.dramas.id, Number(body.drama_id)))
  if (!drama) return notFound(c, '项目不存在')
  const workspaceId = await currentWorkspaceId(c)
  if (drama.workspaceId != null && drama.workspaceId !== workspaceId) return notFound(c, '项目不存在')
  if (!await canWriteDrama(c, Number(body.drama_id))) return c.json({ code: 403, message: '当前角色没有创建剧集的权限' }, 403)
...
app.put('/:id', async (c) => {
  ...
  const [target] = await db.select().from(schema.episodes).where(eq(schema.episodes.id, id))
  if (target && !await canWriteDrama(c, target.dramaId)) return c.json({ code: 403, message: '当前角色没有编辑剧集的权限' }, 403)
  const body = await c.req.json()
...
app.delete('/:id', async (c) => {
  ...
  if (!await canManageDrama(c, ep.dramaId)) return c.json({ code: 403, message: '只有企业所有者或团队管理员可以删除剧集' }, 403)
...
app.post('/:id/extract', async (c) => {
  ...
  if (!ep) return notFound(c, '剧集不存在')
  const token = (c.req.header('Authorization') || '').replace(/^Bearer\s+/i, '').trim()
  const creditOwner = await workspaceForToken(token, Number(c.req.header('X-Workspace-Id') || 0) || undefined)
  if (!creditOwner) return badRequest(c, '未找到可用积分账户')
  const extractCost = await getPrice('asset_extract')
  try {
    const started = await startExtraction(ep.id, ep.dramaId, target, { model: body.model || undefined, configId: body.config_id ?? undefined, credit: { workspaceId: creditOwner.workspaceId, userId: creditOwner.userId } })
    return success(c, { target, status: 'running', already_running: !started, credit_cost: extractCost })
  } catch (error: any) { return badRequest(c, error.message || '积分不足') }
})
...
app.post('/:id/generate-video-prompts', async (c) => {
  ...
  let result
  try {
    result = await startVideoPromptBatch(ep.id, ep.dramaId, { model: body.model || undefined, configId: body.config_id ?? undefined, credit: { workspaceId: creditOwner.workspaceId, userId: creditOwner.userId } }, storyboardIds)
  } catch (error: any) { return badRequest(c, error.message || '积分不足') }
  ...
  const promptUnitPrice = await getPrice('video_prompt')
  return success(c, { status: 'running', total: result.total, credit_cost: promptUnitPrice * result.total, unit_credit_cost: promptUnitPrice })
})
...
app.get('/:id/generation-tasks', async (c) => {
  ...
  const tasks = allTasks
    .filter(t =>
      (t.type === 'text' && taskEpisodeId(t) === episodeId) ||
