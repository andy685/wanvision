...
async function getDefaultTextModel() {
  try {
    return (await getTextConfig()).model || ''
  } catch {
    return ''
  }
}
...
app.get('/', async (c) => {
  ...
  const list = await Promise.all(validAgentTypes.map(async (type) => {
    ...
    return {
      ...
      model,
      effective_model: model || defaultTextModel,
      model_source: model ? 'agent' : 'default_text',
      is_default: !file,
    }
  }))
...
app.get('/:type', async (c) => {
  ...
  return success(c, {
    ...
    model: '',
    effective_model: defaultTextModel,
    model_source: 'default_text',
