...
import { mirrorToObjectStorage, removeFromObjectStorage } from '../services/object-storage.js'
...
export async function downloadFile(url: string, subDir: string): Promise<string> {
  ...
  await mirrorToObjectStorage(`static/${subDir}/${filename}`, buffer, resp.headers.get('content-type') || undefined)
...
export async function saveUploadedFile(data: ArrayBuffer, subDir: string, originalName: string): Promise<string> {
  ...
  await mirrorToObjectStorage(`static/${subDir}/${filename}`, Buffer.from(data))
  return `static/${subDir}/${filename}`
}
...
/** 删除项目关联的本地生成文件；只允许删除 STORAGE_ROOT 下的路径。 */
export function removeStoredFiles(relativePaths: Array<string | null | undefined>) {
  const root = path.resolve(STORAGE_ROOT)
  for (const relativePath of relativePaths) {
    if (!relativePath || /^https?:\/\//i.test(relativePath)) continue
    const target = path.resolve(getAbsolutePath(relativePath))
    if (!target.startsWith(`${root}${path.sep}`)) continue
    for (const file of [target, thumbPathFor(target)]) {
      try { if (fs.existsSync(file)) fs.unlinkSync(file) } catch (error) { console.warn(`[storage] failed to remove ${file}: ${(error as Error).message}`) }
    }
    void Promise.all([relativePath, thumbPathFor(relativePath)].map(file => removeFromObjectStorage(file)))
      .catch(error => console.warn(`[storage] failed to remove remote object: ${(error as Error).message}`))
  }
}
...
export async function saveBase64Image(base64Data: string, mimeType: string, subDir: string): Promise<string> {
  ...
  fs.writeFileSync(filePath, buffer)
  await mirrorToObjectStorage(`static/${subDir}/${filename}`, buffer, mimeType)
