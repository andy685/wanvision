        ...
        <div v-if="tab === 'ai'" class="settings-scroll">
          ...
          <section class="card quick-card">
            <div class="quick-card-head">
              <div class="setup-title">自有 API 快捷配置</div>
              <span class="tag tag-accent">推荐</span>
            </div>
            <p class="setup-desc">
              输入自有 API Key，一次写入文本、图片、视频服务配置。
              <a class="huobao-site-link" href="https://cloudapi.flowingcloud.com" target="_blank" rel="noopener noreferrer">
                打开自有 API 控制台
                <ExternalLink :size="12" :stroke-width="1.8" />
              </a>
            </p>
            <div class="huobao-quick-row">
              <input v-model="huobaoApiKey" class="input" type="password" placeholder="自有 API Key" />
              ...
              <div v-for="s in currentSkills" :key="s.id" class="card skill-card">
                <div class="skill-card-head" @click="toggleSkillEdit(s.id)">
                  ...
                  <div style="flex:1;min-width:0">
                    <div style="font-weight:600;font-size:13px">{{ skillDisplayName(s) }}</div>
      ...
      <form class="dialog config-dialog" @submit.prevent="saveCfg">
        ...
        <div class="dialog-body config-dialog-body">
          ...
          <label class="field">
            ...
            <input v-model="cfgForm.name" class="input" placeholder="如 万影默认图像服务" />
          </label>
...
async function applyHuobaoQuickConfig() {
  ...
  if (!apiKey) { toast.warning('请填写自有 API Key'); return }
...
const textModelSelectOptions = computed(() =>
  [
    { label: '', options: [{ label: '使用 AI 服务默认', value: '' }] },
...
onMounted(() => {
  ...
  loadCfgs(); loadAgents(); loadAllSkills(); loadStylePresets()
})
</script>
