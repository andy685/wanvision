  ...
  <div class="page">
    ...
    <div v-else-if="filteredDramas.length" class="project-grid">
      ...
      >
        <div class="project-thumb" aria-hidden="true">
          ...
          <div class="status-wrap" @click.stop @pointerdown.stop>
            <button type="button" class="cover-badge tag status-badge" title="点击标记项目状态" @click="toggleStatusMenu(d.id)">
              <span class="status-dot" :class="statusDotClass(d)"></span>
              {{ projectStatus(d) }}
            </button>
            <div v-if="statusMenuId === d.id" class="more-menu status-menu" @pointerdown.stop>
              <button
          ...
          <div class="more-wrap" @pointerdown.stop>
            ...
            <div v-if="activeMenuId === d.id" class="more-menu project-menu" @click.stop @pointerdown.stop>
              <button type="button" class="menu-item primary" @click="continueDrama(d)">
                <PlayCircle :size="15" :stroke-width="1.8" />
                继续制作
              </button>
              <button type="button" class="menu-item" @click="openDrama(d)">
                <FolderOpen :size="15" :stroke-width="1.8" />
                项目概览
              </button>
              <button type="button" class="menu-item" @click="copyDramaName(d)">
                <Copy :size="15" :stroke-width="1.8" />
                复制名称
              </button>
              <div class="menu-divider"></div>
              <div class="menu-label">项目状态</div>
              <button
                v-for="s in statusOptions"
                :key="s.value"
                type="button"
                ...
                @click="setDramaStatus(d, s.value)"
              >
                <Check :size="15" :stroke-width="2" :class="{ invisible: currentStatus(d) !== s.value }" />
                {{ s.label }}
              </button>
              <div class="menu-divider"></div>
              <button type="button" class="menu-item is-danger" @click="activeMenuId = null; dramaToDelete = d">
    ...
    <div v-else class="empty-state">
      <div class="empty-icon">
        <SquarePlus :size="28" :stroke-width="1.3" />
      </div>
      ...
      <div class="dialog create-dialog">
        <div class="dialog-head">
          <div class="modal-icon">
            <SquarePlus :size="18" :stroke-width="1.8" />
          </div>
...
import { Check, Clock, Copy, Film, FolderOpen, MoreHorizontal, PlayCircle, Plus, Search, SquarePlus, Trash2 } from 'lucide-vue-next'
...
function toggleMenu(id) {
  ...
  if (activeMenuId.value) statusMenuId.value = null
}

function toggleStatusMenu(id) {
  statusMenuId.value = statusMenuId.value === id ? null : id
  if (statusMenuId.value) activeMenuId.value = null
}
...
function continueDrama(d) {
  activeMenuId.value = null
  statusMenuId.value = null
  navigateTo(`/drama/${d.id}/episode/${getEpisodeNumber(d)}`)
}

async function copyDramaName(d) {
  activeMenuId.value = null
  statusMenuId.value = null
  try {
    await navigator.clipboard.writeText(d.title || '')
    toast.success('项目名称已复制')
  } catch {
    toast.error('复制失败')
  }
}
...
.menu-item.primary { color: var(--accent-text); }
.menu-item.on { background: var(--accent-bg); color: var(--accent-text); }
.menu-divider {
  height: 1px;
