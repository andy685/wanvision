          ...
          <div class="step-toolbar">
            ...
            <div class="toolbar-right">
              ...
              <button v-if="scriptContent" class="btn btn-sm" @click="doRewrite" :disabled="rn">
                ...
                重新改写 <span class="credit-cost">✦ {{ priceFor('script_rewrite') }}</span>
              </button>
            </div>
          </div>
          ...
          <div v-if="!scriptContent && !rn" class="step-empty">
            ...
            <div class="step-empty-actions">
              <button class="btn btn-primary" @click="doRewrite">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
                开始改写 <span class="credit-cost">✦ {{ priceFor('script_rewrite') }}</span>
              </button>
            ...
            <div class="prod-section-bar">
              ...
              <div class="ml-auto flex gap-1 asset-bar-actions">
                ...
                >
                  ...
                  <span class="credit-cost">✦ {{ priceFor('asset_extract') }}</span>
                </button>
                  ...
                  <div class="character-asset-overview"><div class="character-portrait">
                    ...
                    <div class="character-asset-head">
                      ...
                      <button class="btn btn-sm character-gen-btn" :disabled="isPendingCharImage(c.id)" @click.stop="genCharImg(c.id)">
                        ...
                        <span v-if="!isPendingCharImage(c.id)" class="credit-cost">✦ {{ priceFor('character_image') }}</span>
                      </button>
              ...
              >
                ...
                <div class="asset-foot">
                  ...
                  <button class="btn btn-sm" :disabled="isPendingSceneImage(s.id)" @click.stop="genSceneImg(s.id)">
                    ...
                    <span v-if="!isPendingSceneImage(s.id)" class="credit-cost">✦ {{ priceFor('scene_image') }}</span>
                  </button>
                </div>
              </div>
            </div>
              ...
              >
                ...
                <div class="asset-foot">
                  ...
                  <button class="btn btn-sm" :disabled="isPendingPropImage(p.id)" @click.stop="genPropImg(p.id)">
                    ...
                    <span v-if="!isPendingPropImage(p.id)" class="credit-cost">✦ {{ priceFor('prop_image') }}</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div class="prod-section-bar">
              ...
              <div class="ml-auto flex gap-1">
                <button class="btn btn-sm" :disabled="rn" @click="doBreakdown">
                  ...
                  {{ sbs.length ? '重新拆分' : '开始拆分' }} <span class="credit-cost">✦ {{ priceFor('storyboard_break') }}</span>
                </button>
              ...
              <aside class="storyboard-shot-list">
                ...
                <div v-if="sbSelectMode" class="shot-select-bar">
                  ...
                  <button class="btn btn-sm btn-primary shot-select-go" :disabled="!selectedSbIds.length || videoPromptBatch.running" @click="generateSelectedVideoPrompts">
                    <Loader2 v-if="videoPromptBatch.running" :size="11" class="animate-spin" />
                    {{ videoPromptBatch.running ? `生成中 ${videoPromptBatch.completed}/${videoPromptBatch.total}` : `生成视频提示词(${selectedSbIds.length})` }} <span v-if="!videoPromptBatch.running" class="credit-cost">✦ {{ selectedSbIds.length * priceFor('video_prompt') }}</span>
                  </button>
                </div>
              </aside>
                    ...
                    <div class="detail-section">
                      <div class="detail-section-head">
                        ...
                        >
                          ...
                          {{ (selectedSb.video_prompt || selectedSb.videoPrompt) ? '重新生成' : 'AI 生成' }} <span class="credit-cost">✦ {{ priceFor('video_prompt') }}</span>
                        </button>
                      </div>
          ...
          <div v-if="prodTab === 'videos'" class="prod-content">
            <div class="prod-section-bar">
              ...
              <div class="ml-auto flex gap-1">
                ...
                <button class="btn btn-sm" :disabled="!sbs.length" @click="batchVideos">
                  ...
                  批量视频 <span class="credit-cost">✦ {{ batchVideoPrice }}</span>
                </button>
              </div>
            </div>
            <div v-if="!sbs.length" class="step-empty video-task-empty-state">
              ...
              <button class="btn btn-primary" :disabled="rn" @click="prodTab = 'storyboard'; doBreakdown()">
                <Loader2 v-if="rt === 'storyboard_breaker'" :size="13" class="animate-spin" />
                <svg v-else width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
                AI 生成分镜 <span class="credit-cost">✦ {{ priceFor('storyboard_break') }}</span>
              </button>
            </div>
              ...
              <aside class="video-task-inspector">
                <div class="video-inspector-body">
                  ...
                  <button
                    ...
                    @click="genVid(selectedSb)"
                  >
                    {{ videoTaskActionLabel(selectedSb) }} <span class="credit-cost">✦ {{ currentVideoPrice }}</span>
                  </button>
                </div>
              </aside>
              </div>
          ...
          <div v-else class="video-task-table task-drawer-body">
            ...
            >
              <div class="video-task-preview">
                ...
                />
                <div v-else-if="row.kind === 'text'" class="video-task-text-cover">
                  <FileText :size="20" :stroke-width="1.6" />
                  <strong>{{ genTaskTextCover(row) }}</strong>
                </div>
                <div v-else class="video-task-empty">
              ...
              <div class="asset-detail-section-title">
                ...
                <div class="asset-detail-prompt-head-actions">
                  ...
                  >
                    ...
                    {{ isGeneratingPrompt(assetDetail.type, assetDetail.item.id) ? '生成中' : (assetFinalPrompt ? '重新生成' : '生成提示词') }} <span v-if="!isGeneratingPrompt(assetDetail.type, assetDetail.item.id)" class="credit-cost">✦ {{ priceFor(`${assetDetail.type}_prompt`) }}</span>
                  </button>
...
const fallbackPricing = Object.fromEntries([
  ...
  ['video_prompt', 1], ['character_image', 8], ['scene_image', 6],
...
function videoFailMessage(id) {
  return friendlyErrorMessage(failedVideoMessages.value[id] || '', '')
}
...
async function loadGenTasks() {
  ...
  try {
    ...
    for (const [sbId, t] of latestBySb) {
      ...
      else if (t.status === 'failed') failed[sbId] = friendlyErrorMessage(t.error_msg || '生成失败')
    }
...
function genTaskTargetLabel(t) {
  if (t.type === 'text') return genTaskTextLabel(t)
  if (t.storyboard_id) {
...
function genTaskKindLabel(kind) {
  return kind === 'image' ? '图片' : kind === 'video' ? '视频' : kind === 'text' ? '文本' : '合并'
}

function genTaskTextCover(row) {
  return row.targetLabel.replace('生成', '').slice(0, 4) || '文本'
}
  ...
  for (let i = 0; i < 120; i++) {
    ...
    try {
      ...
      if (res?.status === 'failed') {
        ...
        failedVideoMessages.value = {
          ...failedVideoMessages.value,
          [storyboardId]: friendlyErrorMessage(res?.error_msg || res?.errorMsg || '视频生成失败'),
        }
...
async function doMerge(ids) {
  ...
  const poll = setInterval(async () => {
    ...
    if (mergeData.value?.status === 'completed' || mergeData.value?.status === 'failed') {
      ...
      toast.error(friendlyErrorMessage(mergeData.value?.error_msg || mergeData.value?.errorMsg || '拼接失败'))
      }
    }
  }, 3000)
}
...
.studio-credit-chip:hover { border-color: var(--accent); background: var(--bg-hover); }
.studio-credit-chip span { color: var(--accent); font-size: 13px; }
.studio-topbar .btn {
  height: 26px;
  padding: 0 9px;
  font-size: 10.5px;
...
.video-inspector-action { display: inline-flex; align-items: center; justify-content: center; gap: 2px; }
.video-inspector-action .credit-cost { color: #fff; }
.btn-primary .credit-cost { color: #fff; }
...
.asset-btn-batch:hover .credit-cost,
.asset-btn-batch:focus-visible .credit-cost,
.asset-btn-batch:active .credit-cost { color: #fff; }
...
.video-task-text-cover {
  ...
  gap: 3px;
  background: linear-gradient(135deg, rgba(239,108,79,0.11), rgba(18,24,38,0.04));
  color: var(--accent-text);
}
.video-task-text-cover strong {
  max-width: 72px;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
