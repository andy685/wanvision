                  ...
                  <div class="character-asset-overview">
                    ...
                    <div class="character-asset-head">
                      ...
                      <button class="btn btn-sm character-gen-btn" type="button" :disabled="isPending(m)" @click.stop="generateMaterial(m)">
                        ...
                        <span v-if="!isPending(m)" class="credit-cost">✦ {{ materialPrice(m) }}</span>
                      </button>
            ...
            <div v-else class="asset-grid">
              ...
              >
                ...
                <div class="asset-foot">
                  ...
                  <a v-if="matHasImage(m)" class="asset-download" :href="assetSrc(m)" download @click.stop>下载原图</a>
            ...
            <section class="mat-detail-prompt-panel">
              <div class="mat-detail-section-title">
                ...
                >
                  ...
                  <span v-if="!finalPromptGen" class="credit-cost">✦ {{ priceFor(`${editTarget?.kindKey || 'character'}_prompt`) }}</span>
                </button>
              </div>
              <textarea
          ...
          <footer class="dialog-foot mat-detail-foot">
            ...
            <div class="mat-detail-primary-actions">
              ...
              >
                ...
                <span v-if="!isPending(editTarget)" class="credit-cost">✦ {{ materialPrice(editTarget) }}</span>
              </button>
...
import { dramaAPI, episodeAPI, characterAPI, sceneAPI, propAPI, uploadAPI, pricingAPI } from '~/composables/useApi'
...
async function loadPricing() {
  try { pricingRules.value = await pricingAPI.list() || [] } catch { /* 价格读取失败时使用默认价格 */ }
}

onMounted(() => { load(); loadPricing() })
</script>
...
.asset-download { color: var(--text-2); font-size: 11px; text-decoration: none; }
.asset-download:hover { color: var(--accent); }
.head-secondary-action { margin-left: auto; display: inline-flex; align-items: center; gap: 5px; }
.head-action { margin-left: 0; }
.member-dialog { width: min(100%, 560px); }
.dialog-subtitle { margin-top: 5px; color: var(--text-3); font-size: 11px; }
.member-invite { display: grid; grid-template-columns: 1.4fr 1fr auto; align-items: end; gap: 10px; margin-bottom: 16px; padding-bottom: 16px; border-bottom: 1px solid var(--border); }
.member-invite .btn { min-height: 36px; white-space: nowrap; }
.project-member-list { border: 1px solid var(--border); border-radius: 7px; overflow: hidden; }
.project-member-row { display: flex; align-items: center; gap: 10px; min-height: 54px; padding: 0 12px; border-bottom: 1px solid var(--border); }
.project-member-row:last-child { border-bottom: 0; }
.project-member-row div { display: grid; gap: 3px; flex: 1; }
.project-member-row small { color: var(--text-3); font-size: 11px; }
.member-avatar { display: grid; place-items: center; width: 30px; height: 30px; border-radius: 50%; background: var(--bg-hover); color: var(--text-2); font-size: 10px; }
.remove-member { border: 0; background: transparent; color: #d92d20; cursor: pointer; font-size: 11px; }
.members-loading { padding: 28px 0; text-align: center; color: var(--text-3); font-size: 12px; }
...
@media (max-width: 860px) {
  ...
  .member-invite { grid-template-columns: 1fr; align-items: stretch; }
}
</style>
