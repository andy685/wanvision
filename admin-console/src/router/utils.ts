...
function initRouter() {
  const resolveWithLocalRoutes = (resolve: (router: RouterInstance) => void) => {
    handleAsyncRoutes([]);
    resolve(router);
  };

  if (getConfig()?.EnableAsyncRoutes !== true) {
    return new Promise(resolve => {
      resolveWithLocalRoutes(resolve);
    });
  }
    ...
    if (asyncRouteList && asyncRouteList?.length > 0) {
      ...
      return new Promise(resolve => {
        getAsyncRoutes()
          ...
          .catch(() => resolveWithLocalRoutes(resolve));
      });
    }
  } else {
    return new Promise(resolve => {
